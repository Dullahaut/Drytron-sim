...
#import csv
"""
manif = open('C:/Users/denca/.spyder-py3/failstates.csv', 'w', newline='')
manifestwriter = csv.writer(manif, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
manifestwriter.writerow(['#hand1','#hand2','#hand3','#hand4','#hand5'])
manif_currentrow = []
"""
...
def diagnosing():
    ...
    if a == 1 and (z == 1 or g == 1 or d == 1 or em == 1 or no == 1 or ben > 1 or (ben == 0 and (pain ==1 or met == 1))):
        combo += 1
        ...
        combo += 1
    elif d == 1 and ((met == 1 or div == 1) and ben > 0) or (met == 1  and (ben > 0 or div == 1)):
        combo += 1
    elif d == 1 and z == 1 and g == 1:
        combo += 1
    elif d == 1 and ben > 1 and pain == 1:
        combo += 1
    elif no == 1 and pain == 1 and veil == 1:
        combo += 1
    elif fool == 1 and (div == 1 or ben > 0):
        combo += 1
    elif trap == 1 and (d == 1 or z == 1) and (g == 1 or jack == 1):
        combo += 1
    elif trap == 1 and z == 1 and g == 1:
        combo += 1
        ...
        combo += 1
    elif trap == 1 and d == 1 and pain == 1 and ben > 0:
        combo += 1
...
def recording():
    ...
    if combo == 0:
        if a == 0 and z == 0 and g == 0 and d == 0 and no == 0 and em == 0:
            nonames += 1
        elif ((a + z + g + d + no + em) <= 0) and ((ben + drac + nat) == 0):
            nonames += 1
        elif ne >= 4:
            nonames += 1
        elif ne >= 3 and (pain == 1 or drac == 1):
            nonames += 1
        elif ((g == 1 or em == 1 or no == 1) and z == 1) or (g == 1 and (z == 1 or em == 1 or no == 1)):
            backups += 1
        elif d == 1 and ben == 1 and pain == 1:
            backups += 1
        elif a == 1 and ben == 1:
        ...
        """
            ...
            manif_currentrow.clear()
            for j in range(0,5):
                manif_currentrow.append(hand[j])
            manifestwriter.writerow(manif_currentrow)
        """
    ...
    if prosp == 1:
        if combo == 0:
            ...
            if a_nib == 0:
                hand.pop(5)
                prospextra += 1 
        elif combo > 0 and ('ne' in prospreveal) or ('ne1' in prospreveal):
            ne += 1
        elif combo > 0:
            prospextra += 1
...
backupper = (backups/count)*100
prospextraper = (prospextra/count)*100
"""
print(eyy)
print(playable)
"""
...
print("percentage hands that must use the backup combo: %d" % backupper + "%" + " %d" % backups)
print("percentage hands that prosp is extraneous: %d" % prospextraper + "%" + " %d" % prospextra)
"""
